...
exports.checkDirectHorseResults = exports.calculateDaysBetween = exports.fetchLastRaceDate = void 0;
const __1 = require("../../../..");
/**
 ...
 */
const fetchLastRaceDate = (horseId, currentDate) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        console.log(`\n[DEBUG] Iniciando busca de última corrida para cavalo ${horseId}`);
        console.log(`[DEBUG] Data atual da corrida: ${currentDate}`);
        if (!horseId) {
            console.log(`[ERRO] ID do cavalo inválido para busca de última corrida: ${horseId}`);
            return null;
        }
        // Primeiro, precisamos encontrar o stats_id do cavalo
        console.log(`[DEBUG] Buscando stats_id para cavalo ${horseId} na tabela horse_stats_hr`);
        const { data: horseStats, error: statsError } = yield __1.supabase
            ...
            .single();
        if (statsError) {
            console.log(`[ERRO] Erro ao buscar stats_id para cavalo ${horseId}: ${statsError.message}`);
        ...
        console.log(`[DEBUG] Stats_id encontrado para cavalo ${horseId}: ${horseStats.id}`);
        // Agora buscamos a data da última corrida usando SQL nativo para garantir formatação correta
        console.log(`[DEBUG] Chamando função SQL get_last_race_date para stats_id ${horseStats.id}`);
        const { data: lastRaceData, error: lastRaceError } = yield __1.supabase.rpc("get_last_race_date", {
            stats_id_param: horseStats.id,
            current_date_param: currentDate,
        });
        if (lastRaceError) {
            console.log(`[ERRO] Erro ao buscar última corrida para cavalo ${horseId} (stats_id ${horseStats.id}): ${lastRaceError.message}`);
            console.log(`[ERRO] Detalhes: ${JSON.stringify(lastRaceError)}`);
            return null;
        }
        console.log(`[DEBUG] Resultado da função SQL: ${JSON.stringify(lastRaceData)}`);
        if (!lastRaceData || lastRaceData.length === 0) {
            console.log(`[AVISO] Nenhum resultado retornado da função SQL para cavalo ${horseId} (stats_id ${horseStats.id})`);
            return null;
        }
        if (!lastRaceData[0].max_date) {
            console.log(`[AVISO] Função SQL retornou objeto sem max_date para cavalo ${horseId} (stats_id ${horseStats.id})`);
            return null;
        }
        console.log(`[DEBUG] Data da última corrida encontrada para cavalo ${horseId}: ${lastRaceData[0].max_date}`);
        return lastRaceData[0].max_date;
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.log(`[ERRO] Exceção ao buscar data da última corrida para cavalo ${horseId}: ${errorMessage}`);
        console.log(`[ERRO] Stack trace: ${error instanceof Error ? error.stack : "Não disponível"}`);
        return null;
    }
});
exports.fetchLastRaceDate = fetchLastRaceDate;
/**
...
const calculateDaysBetween = (startDate, endDate) => {
    try {
        ...
        if (!startDate || !endDate) {
            console.log(`[AVISO] Datas inválidas para cálculo: startDate=${startDate}, endDate=${endDate}`);
            return 0;
        }
        ...
        console.log(`[DEBUG] Resultado final (após garantir não-negativo): ${result} dias`);
        return result;
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
 ...
 */
const checkDirectHorseResults = (horseId, currentDate) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        console.log(`[DEBUG] Verificando diretamente horse_results_hr para cavalo ${horseId}`);
        ...
        const { data: results, error } = yield __1.supabase
            ...
            .limit(1);
        if (error) {
            console.log(`[ERRO] Erro ao buscar diretamente em horse_results_hr: ${error.message}`);
            return null;
        }
        if (!results || results.length === 0) {
            console.log(`[AVISO] Nenhum resultado direto encontrado para cavalo ${horseId}`);
            return null;
        }
        console.log(`[DEBUG] Resultado direto encontrado: ${JSON.stringify(results[0])}`);
        ...
        if (resultDateParts.length !== 3) {
            console.log(`[ERRO] Formato de data inválido no resultado: ${results[0].date}`);
            return null;
        }
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.log(`[ERRO] Exceção ao verificar diretamente horse_results_hr: ${errorMessage}`);
        return null;
    }
});
exports.checkDirectHorseResults = checkDirectHorseResults;
