...
const populateHorseStats_spb = (next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        for (const stats of horseStats) {
            const { data: existing, error: checkError } = yield __1.supabase
                .from("horse_stats_hr")
                .select("id")
            ...
            if (existing && existing.length > 0) {
                ...
                const { data: insertedStats, error: insertError } = yield __1.supabase
                    .from("horse_stats_hr")
                    .insert({
                    horse: stats.horse,
            ...
            for (const results of stats.results) {
                const { data: existingResult, error: resultCheckError } = yield __1.supabase
                ...
                if (existingResult && existingResult.length > 0) {
                    ...
                    const { error: insertResultError } = yield __1.supabase
                        .from("horse_results_hr")
                        .insert({
                        stats_id: stats_id,
                        date: results.date,
