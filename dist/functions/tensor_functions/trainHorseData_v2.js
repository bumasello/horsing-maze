...
exports.trainHorseData_v2 = void 0;
const tf = __importStar(require("@tensorflow/tfjs-node"));
const __1 = require("../.."); // Ajuste o caminho conforme necessário
...
function getLatestModelPathFromSupabase() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (data && data.length > 0) {
            const versionFolders = data.filter((item) => !item.id && item.name !== ".emptyFolderPlaceholder");
...
function saveNormalizationParams(xMean, xStd, featureKeys) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const normParamsPath = path.join(LOCAL_TEMP_MODEL_DIR, "normalization.json");
            yield fs.promises.writeFile(normParamsPath, JSON.stringify(normalizationParams), "utf8");
...
const trainHorseData_v2 = () => __awaiter(void 0, void 0, void 0, function* () {
    let model = undefined;
    let xMean = undefined;
    let xStd = undefined;
    let xTrainTensorRaw = undefined;
    let yTrainTensor = undefined;
    let normalizedXTrainTensor = undefined;
    try {
        console.log("Iniciando treinamento do modelo com dados da tabela training_horse_features...");
        ...
        const { data: trainingEntries, error: trainingError } = yield __1.supabase
            .schema("hml")
            .from("training_horse_features")
            .select(selectColumns);
        if (trainingError) {
            throw new Error(`Erro ao buscar dados de treinamento: ${JSON.stringify(trainingError)}`);
        }
        if (!trainingEntries || trainingEntries.length === 0) {
            console.log("Sem dados de treinamento suficientes na tabela training_horse_features.");
            return;
        }
        console.log(`Encontrados ${trainingEntries.length} registros para treinamento.`);
        console.log(`Features esperadas (${EXPECTED_FEATURE_KEYS.length}): ${EXPECTED_FEATURE_KEYS.join(", ")}`);
        // 2. Extrair features e target usando a lista explícita
        // Fazer cast explícito para o tipo correto
        const typedEntries = trainingEntries;
        const xs_raw = typedEntries.map((e) => EXPECTED_FEATURE_KEYS.map((k) => e[k]));
        const ys_raw = typedEntries.map((e) => { var _a; return (_a = e.target) !== null && _a !== void 0 ? _a : 0; }); // Usar 0 como fallback para null
        // 3. Limpar e processar os dados
        const xs_train_cleaned = xs_raw.map((arr) => arr.map((val) => (val === null || Number.isNaN(val) ? 0 : val)));
        ...
        xTrainTensorRaw = tf.tensor2d(xs_train_cleaned);
        yTrainTensor = tf.tensor2d(ys_train_cleaned, [ys_train_cleaned.length, 1]);
        // 6. Carregar modelo existente ou criar novo
        const latestModelPaths = yield getLatestModelPathFromSupabase();
        let loadedNormalizationParams = null;
        if (latestModelPaths) {
            try {
                ...
                loadedNormalizationParams = yield normResponse.json();
                // *** VALIDAR CONSISTÊNCIA DAS FEATURES ***
                if (JSON.stringify(loadedNormalizationParams === null || loadedNormalizationParams === void 0 ? void 0 : loadedNormalizationParams.featureKeys) !==
                    JSON.stringify(EXPECTED_FEATURE_KEYS)) {
                    throw new Error("Incompatibilidade de features entre o modelo salvo e o código atual. Crie um novo modelo.");
                }
                // Carregar modelo
                ...
                console.log("Erro ao carregar ou validar modelo/normalização do Supabase, criando um novo:", error);
        ...
        if (loadedNormalizationParams &&
            ...
            xMean = tf.tensor1d(loadedNormalizationParams.mean);
            xStd = tf.tensor1d(loadedNormalizationParams.std);
            console.log("Usando parâmetros de normalização carregados do modelo existente.");
        }
        else {
        ...
        if (!xMean || !xStd || !xTrainTensorRaw) {
            throw new Error("Falha ao calcular ou carregar parâmetros de normalização.");
        }
        // Usar type assertions para garantir ao TypeScript que os tensores têm os tipos corretos
        const inputTensor = xTrainTensorRaw; // Garantir que é Tensor2D
        const meanTensor = xMean; // Garantir que é Tensor1D
        const stdTensor = xStd; // Garantir que é Tensor1D
        // Agora podemos usar os tensores com segurança, sem operadores !
        normalizedXTrainTensor = tf.tidy(() => inputTensor.sub(meanTensor).div(stdTensor));
        ...
        const history = yield model.fit(normalizedXTrainTensor, yTrainTensor, {
            epochs: 50,
            batchSize: 32,
            validationSplit: 0.2,
            callbacks: {
        ...
        const finalAccValue = finalAcc instanceof tf.Tensor
            ? finalAcc.dataSync()[0]
            : finalAcc;
        console.log(`Acurácia final: ${finalAccValue !== undefined ? finalAccValue.toFixed(4) : "N/A"}`);
        // 8. Salvar o modelo e os parâmetros de normalização
        try {
            yield ensureDir(LOCAL_TEMP_MODEL_DIR);
            // Salvar parâmetros de normalização
            yield saveNormalizationParams(xMean, xStd, EXPECTED_FEATURE_KEYS);
            // Salvar o modelo
        ...
        ].filter((t) => t !== undefined); // Filtrar undefined antes de dispose
...
exports.trainHorseData_v2 = trainHorseData_v2;
