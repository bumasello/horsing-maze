...
function applyBaselineFilter(feats: string[]): string[] {
	...
	if (mode === "no_market")
		return feats.filter((f) => !NO_MARKET_FEATURES.has(f));
...
export async function trainLayBettingModel(
	...
	if (baselineMode) {
		console.log(
			`🧪 BASELINE_MODE=${baselineMode} — save isolado, sem bump de versão prod\n`,
		);
	}
	if (experimentLabel) {
		console.log(
			`🧪 EXPERIMENT_LABEL=${experimentLabel} — save isolado em baselines/${experimentLabel}_${modelType}\n`,
		);
	}
	if (multiTaskEnabled) {
		console.log(
 ...
 */
function bceLoseLoss(loseSigmoid: tf.Tensor3D, batchY: tf.Tensor2D): tf.Scalar {
	return tf.tidy(() => {
 ...
 */
function calculateBrierScore(probs: tf.Tensor2D, targets: tf.Tensor2D): number {
	return tf.tidy(() => {
		const mask = targets.greaterEqual(0).toFloat() as tf.Tensor2D;
		const targetsClean = targets.maximum(0) as tf.Tensor2D;
...
function calculateECE(
	...
	return tf.tidy(() => {
		...
		const correct = predictedWinner
			.equal(actualWinner)
			.toFloat() as tf.Tensor1D;
		...
		for (let b = 0; b < numBins; b++) {
			...
			for (let i = 0; i < N; i++) {
				...
				const inBin =
					b === numBins - 1 ? c >= lo && c <= hi : c >= lo && c < hi;
				if (inBin) {
					binCount++;
					confSum += c;
					accSum += correctArr[i];
