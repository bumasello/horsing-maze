...
import { pendingRaces, loadTrainingData } from "./loadData";
import { supabase } from "../..";
import type { IHorseFeatureEntry_Spb } from "../../models/modelSpb/horseFeatureEntry_Spb";

export const cl_trainData = async () => {
  ...
  const featureKeys = Object.keys(trainingData[0]).filter(
    (k) =>
      ![
        "id",
        "race_horse_id",
  ...
  // Treinar o modelo
  const history = await model.fit(normalizedXs, yTensor, {
    epochs: 50,
    batchSize: 32,
    validationSplit: 0.2,
    callbacks: {
  ...
  console.log(
    `Modelo treinado com acurácia: ${history.history.acc[history.history.acc.length - 1]}`,
  );
  ...
  for (const { raceId, features } of races) {
    ...
    const inputData = features.map((e) =>
      featureKeys
        .map((k) => (e as any)[k] as number)
    ...
    features.forEach((f, i) => {
      console.log(
        `Cavalo ${f.race_horse_id}: ${(probArr[i] * 100).toFixed(2)}%`,
      );
    });
  }
